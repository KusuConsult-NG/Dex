      ...
      <AppPageBodySection>
        <Tabs variant="soft-rounded" colorScheme="green">
          <Flex direction={{ md: "column", base: "column" }}>
            <InputGroup
              style={{ width: "50%", marginRight: 12, marginBottom: 12 }}
              w={{}}
            >
              <InputLeftElement
                pointerEvents="none"
...
const PoolCard: React.FC<{ pool_props: { label: string; value: string }[] }> =
  ({ pool_props }) => {
    return (
      <Box
        padding="12px"
      ...
      >
        ...
        {pool_props.map((v) => (
          <Flex justify="space-between" margin="12px 0px">
            ...
            <Text fontSize="md" color="rgba(90,90,90,.6)">
              {v.value}
            </Text>
          </Flex>
        ))}
